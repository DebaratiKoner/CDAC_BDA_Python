...
def process_transaction_batch(accounts,batch_list,log_path):
    ...
    try:
        for transaction in batch_list:
            ...
            if amt<=0:
                raise InvalidTransactionError("Transaction amount mustbe positive.")
            if trans_type == "deposit":
                accounts[acc] += amt
            elif trans_type == "withdraw":
                if accounts[acc] < amt:
                    raise OverdraftError(f"Insufficient funds. Account {acc} "
                        f"has balance {accounts[acc]}, "
                        f"requested {amt}."
                    )
                accounts[acc] -= amt
...
print(accounts)
# Result: accounts changes to {"ACC01": 70.0, "ACC02": 70.0}
# transactions.log writes: "[SUCCESS] Batch completed. 2 transaction(s)processed."# Batch 2: Invalid transaction (triggers rollback)
batch_2 = [{"acc": "ACC01", "type": "deposit", "amt": 50.0},
...
print("After Batch 2:")
print(accounts)
 # Output: {"ACC01": 70.0, "ACC02": 70.0}# transactions.log writes: "[ROLLBACK] Batch aborted: OverdraftError -Insufficient funds. Account ACC02 has balance 70.0, requested 200.0."